    ...
    <div class="tabs tabs-full">
        ...
        <form method="post" action="{{ route('LaravelInstaller::environmentSaveWizard') }}" class="tabs-wrap">
            <div class="tab" id="tab1content">
                ...
                <div class="form-group {{ $errors->has('envato_key') ? ' has-error ' : '' }}">
                    <label for="envato_key">
                        {{ trans('installer_messages.environment.wizard.form.app_tabs.purchase_code') }}
                        <sup>
                            <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank" title="{{ trans('installer_messages.environment.wizard.form.app_tabs.more_info') }}">
                                <i class="fa fa-info-circle fa-fw" aria-hidden="true"></i>
                                <span class="sr-only">{{ trans('installer_messages.environment.wizard.form.app_tabs.more_info') }}</span>
                            </a>
                        </sup>
                    </label>
                    <input type="text" name="envato_key" id="envato_key" value="{{old('envato_key')}}" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.envato_key_here') }}" />
                    @if ($errors->has('envato_key'))
                        <span class="error-block">
                            <i class="fa fa-fw fa-exclamation-triangle" aria-hidden="true"></i>
                            {{ $errors->first('envato_key') }}
                        </span>
                    @endif
                </div>
